...
#função usada para baixar novas configurações do json
...
while True:
    ...
    try:
        ...
        #verifica se ja existe o remote_maintenance.json
                            ...
                            for maquina in valor['maquinas']:
                                if maquina == 'all' or maquina == hostname:
                                    if valor['version'] > remote_maintenance_old[chave]['version']:
                                        file_executor(app_name=chave, url=valor['url'], executable_name=valor['executable_name']) # type: ignore
            ...
            print('Não foi possível baixar o JSON inicial')  # Substituir este print por um log

    except Exception as e:
        print(f'Erro durante a execução do loop principal: {e}')  # Substituir este print por um log

    # Aguarda o intervalo definido em time_loop antes de executar novamente
    if remote_maintenance_new:
        time.sleep(remote_maintenance_new['config']['time_loop'])
    else:
        time.sleep(60)
